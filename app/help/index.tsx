...
export default function HelpScreen() {
  ...
  const sections = t('legal.help.sections', { returnObjects: true }) as { title: string; body: string }[];

  return (
    <View style={[styles.safe, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={[styles.content, isDesktopWeb && { maxWidth: maxW, alignSelf: 'center' }]}>
          <Heading level={1} style={styles.title}>{t('legal.help.title')}</Heading>
          <ThemedText style={[styles.meta, { color: colors.icon }]}>{t('legal.help.meta')}</ThemedText>
          {sections.map((s, i) => (
